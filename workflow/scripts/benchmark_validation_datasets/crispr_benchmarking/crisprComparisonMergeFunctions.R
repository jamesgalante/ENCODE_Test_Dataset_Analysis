...
combineAllExptPred <- function(expt, pred_list, config, outdir, fill_pred_na = TRUE, extract_features = FALSE) {
  ...
  merged_list <- lapply(
    ...
    function(p) {
      combineSingleExptPred(expt = expt, pred = pred_list[[p]], pred_name = p, config = config,
                            outdir = outdir, fill_pred_na = fill_pred_na,  extract_features)
    })
...
combineSingleExptPred <- function(expt, pred, pred_name, config, outdir, fill_pred_na, extract_features = FALSE) {
  ...
  # If ENCODE model features are wanted for evaluation, add those columns after merging
  if (extract_features) {
    # Get all column names that end in ".Feature"
    feature_cols <- grep("\\.Feature$", names(pred), value = TRUE)
    # Add PredictionCellType and the prediction column from config
    feature_cols <- c("PredictionCellType", config_filt$pred_col, feature_cols)
    merged <- cbind(expt[queryHits(ovl)], pred[subjectHits(ovl), feature_cols, with = FALSE])
  }
  ...
  # Define columns to use for aggregation
  feature_cols <- if(extract_features) {
    grep("\\.Feature$", colnames(merged), value = TRUE)
  } else {
    NULL
  }
  agg_cols <- setdiff(colnames(merged), c(config_filt$pred_col, feature_cols))  # exclude features from grouping
  merged <- collapseEnhancersOverlappingMultiplePredictions(merged, 
                                                            config = config_filt,
                                                            agg_cols = agg_cols,
  ...
  # If ENCODE model features are wanted for evaluation, add those column names to expt_missing_preds
  if (extract_features) {
    feature_cols <- grep("\\.Feature$", names(pred), value = TRUE)
    expt_missing_preds[, feature_cols] <- NA
  }
...
collapseEnhancersOverlappingMultiplePredictions <- function(df, config, agg_cols, feature_cols = NULL) {
  ...
  # If we have feature columns, handle them using the same grouping structure
  if (!is.null(feature_cols) && length(feature_cols) > 0) {
    for (feat_col in feature_cols) {
      # Get the grouped structure from our existing output
      grouped_data <- df[output, on = agg_cols, nomatch = 0]
      
      # Apply specific aggregation based on feature column type
      agg_result <- if (feat_col == "distanceToTSS.Feature") {
        grouped_data[, setNames(.(mean(get(feat_col))), feat_col), by = agg_cols]
      } else if (feat_col == "normalizedDNase_prom.Feature") {
        grouped_data[, setNames(.(if(length(unique(get(feat_col))) > 1) {
          warning("Different values found for normalizedDNase_prom.Feature in aggregation")
          mean(get(feat_col))
        } else get(feat_col)[1]), feat_col), by = agg_cols]
      } else if (feat_col == "3DContact.Feature") {
        grouped_data[, setNames(.(mean(get(feat_col))), feat_col), by = agg_cols]
      } else if (feat_col == "ABC.Score.Feature") {
        grouped_data[, setNames(.(mean(get(feat_col))), feat_col), by = agg_cols]
      } else if (feat_col == "numCandidateEnhGene.Feature") {
        grouped_data[, setNames(.(round(mean(get(feat_col)))), feat_col), by = agg_cols]
      } else if (feat_col == "numTSSEnhGene.Feature") {
        grouped_data[, setNames(.(round(mean(get(feat_col)))), feat_col), by = agg_cols]
      } else if (feat_col == "sumNearbyEnhancers.Feature") {
        grouped_data[, setNames(.(mean(get(feat_col))), feat_col), by = agg_cols]
      } else if (feat_col == "ubiquitousExpressedGene.Feature") {
        grouped_data[, setNames(.(if(length(unique(get(feat_col))) > 1) {
          warning("Different values found for ubiquitousExpressedGene.Feature in aggregation")
          get(feat_col)[1]
        } else get(feat_col)[1]), feat_col), by = agg_cols]
      }
      
      # Merge the aggregated feature column back into output
      output <- merge(output, agg_result, by = agg_cols)
    }
  }
