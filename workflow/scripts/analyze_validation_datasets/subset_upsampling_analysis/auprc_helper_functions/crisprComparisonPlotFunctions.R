...
# Function from Andreas' dev branch for crisprComparisons which includes a "plot_thresholds" option
makePRCurvePlot <- function(pr_df, pred_config, n_pos, pct_pos, min_sensitivity = 0.7,
                            plot_name = "PRC full experimental data", line_width = 1, 
  ...
  # Filter out rows where precision, recall, or F1 are NaN or Inf
  pr_quant_col <- pr_quant_col %>%
    filter(!is.nan(precision) & !is.infinite(precision) & 
             !is.nan(recall) & !is.infinite(recall) &
             !is.nan(F1) & !is.infinite(F1))
...
applyCellTypes <- function(merged, .fun, ..., cell_types_col = "ExperimentCellType",
  ...
  # get the last cell type
  last_cell_type <- tail(cell_types, n = 1)
  
  # apply function to the last cell type
  result <- lapply(last_cell_type, FUN = applyCellType, merged = merged, .fun = .fun, ...)
  
  # return the result for the last cell type
  return(result)
