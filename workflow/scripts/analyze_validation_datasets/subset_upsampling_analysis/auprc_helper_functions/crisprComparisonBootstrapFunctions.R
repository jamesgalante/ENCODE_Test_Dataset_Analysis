...
bootstrapPerformanceIntervals <- function(data, metric = c("auprc", "precision"), predictors = NULL,
  ...
  #indices_are_one <- seq_along(bs_perf$t0)[bs_perf$t0[c(1,2)] == 1] # indices of stat == 1 that we have to check for NA
  #pred_indices <- which(colSums(is.na(bs_perf$t)) == 0)
  
  # Identify indices of non-NA predictors
  pred_indices <- seq_along(bs_perf$t0)[!is.na(bs_perf$t0)]
  # Identify indices where t0 is equal to 1 and columns contain NA values
  invalid_indices <- which(bs_perf$t0 == 1 & colSums(is.na(bs_perf$t)) > 0)
  # Remove invalid indices from pred_indices
  pred_indices <- setdiff(pred_indices, invalid_indices) 
