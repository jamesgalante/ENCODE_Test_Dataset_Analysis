...
#' @param cell_metadata (optional) A data.frame containing additional cell meta data. The first 
#'   column is assumed to contain cell barcodes.
#' @param targets_pert_name How should the target-level perturbations be called?
#'   (default: "target_perts")
#' @value An SCE object.
create_pert_sce <- function(expr, annot, pert_status, grna_annot, cell_metadata = NULL,
  ...
  cell_stats <- DataFrame(cell_barcode = names(umis_per_cell),
                          total_umis = umis_per_cell,
                          detected_genes = genes_per_cell )
  
  # add any optional cell metadata if provided
  if (!is.null(cell_metadata)) {
    
    # check if there are conflicting column names
    shared_cols <- intersect(colnames(cell_stats), colnames(cell_metadata)[-1])
    if (length(shared_cols) > 0) {
      stop("Invalid column names in cell_metadata:", paste(shared_cols, collapse = ","),
           call. = FALSE)
    }
  ...
  cell_stats <- DataFrame(cell_stats[, -1], row.names = cell_stats[[1]])
