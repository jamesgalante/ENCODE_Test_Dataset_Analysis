# Script: # Script: create_sceptre_diffex_input_K562_DC_TAP.R

### SETUP =====================================================================

# Saving image for debugging
save.image(paste0("RDA_objects/create_sceptre_diffex_input_K562_DC_TAP.rda"))
message("Saved Image")
...
dge <- as(as.matrix(dge, rownames = 1), "sparseMatrix")
# Import the perturbation status dataframe
perturb_status <- fread(snakemake@input$perturb_status)
perturb_status <- as(as.matrix(perturb_status, rownames = 1), "sparseMatrix")
# Import the annotation file
annot <- import(snakemake@input$annot)
# Import the guide_targets file
guide_targets <- read_tsv(snakemake@input$guide_targets) %>% filter(target_type == "enh")


### CREATE GENE_GRNA_GROUP_PAIRS ==============================================

# Create the file
target_search_results <- find_genes_near_targets(guide_targets %>% filter(!is.na(target_chr)), annot, rownames(dge), max_distance=1e6)
