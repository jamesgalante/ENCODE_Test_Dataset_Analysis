...
This R markdown file imports the raw count data for the Xie paper from the h5 files in the GSE. Then the perturb status matrices are formatted.

-   All the batches are first normalized using the meta-cell technique in the Xie paper.

-   Each counts matrix is turned into a Seurat object and the binarized perturb status is added as an assay

-   Then all Seurat objects are combined. The barcodes are made unique in this process
...
Then we need to filter both the seurat objects (i don't know if the UMI matrix would have a cell barcode that's not in the seurat object) and the UMI matrix.

Then we need to add the UMI matrix to the seurat object as an assay

```{r}
import_and_format_UMI_matrix <- function(file_path) {
  
  # Import the text file from the file path
...
```{r}
union_row_names <- Reduce(union, lapply(seurat_objects, 
                                        function(x) rownames(x[["UMI_matrix"]])))

create_BLAT_input(sequence_list = union_row_names, 
                  output_path = "../Data/guide_info/to_be_BLAT.fa")
```
...
guide_info <- create_guide_info_from_BLAT(sequence_list = union_row_names,
                                          BLAT_output_path = "../Data/guide_info/out.psl")
```
...
guide_targets <- create_guide_targets(guide_info = guide_info, cCRE_file_path = "../Data/guide_info/sample1_candidate_cres.bed")
```
...
Note that the UMI matrices had already been filtered and pre-processed. Now we're just adding a condition to count these reads as in the cell if there is greater than 1 read

```{r}
binarize_umi_matrix <- function(merged_seurat) {
    
  # Create a dense matrix from the assay for vectorized operations
  dense_umi_matrix <- as.matrix(merged_seurat[["UMI_matrix"]]@counts)
  
  # Initialize the perturb_status matrix
  perturb_status <- as.integer(dense_umi_matrix > 1)
  
  # Reshape the perturb_status to match the dimensions of temp
  perturb_status <- matrix(perturb_status, 
                           nrow = nrow(dense_umi_matrix), 
                           ncol = ncol(dense_umi_matrix), 
                           byrow = FALSE)
  ...
  rownames(perturb_status) <- rownames(merged_seurat[["UMI_matrix"]]@counts)
  colnames(perturb_status) <- colnames(merged_seurat[["UMI_matrix"]]@counts)
  
  # Add the perturb_status matrix as a new assay
  merged_seurat[["perturb_status"]] <- CreateAssayObject(counts = perturb_status)
  
  return(merged_seurat)
}

final_merged_seurat <- binarize_umi_matrix(merged_seurat)
```
...
count_matrix <- GetAssayData(object = JoinLayers(final_merged_seurat), layer = "counts")
...
fwrite(dt, file = "../Data/Final_CRISPRi_Inputs/dge.txt.gz", sep = "\t", quote = FALSE)
rm(dt)
gc()
```
...
```{r}
# Extract matrix
data_matrix <- as.matrix(GetAssayData(object = final_merged_seurat@assays$UMI_matrix, layer = "counts"))

# Convert matrix to data frame and add row names as "VECTOR" column without the spacer
df <- as.data.frame(data_matrix)

# We have guide_targets which is a table that has a column called "spacer", currently, the rownames of the df are named by a spacer that exists in guide_targets. I want to take each rowname in df, and replace it with the corresponding "spacer" row in guide_targets. I want to replace it with the "name" column entry for that row.
...
metadata_df <- tibble(
  cell_barcode = colnames(final_merged_seurat),
  # Extract everything up to the first underscore
  cell_batches = sub("_.*", "", colnames(final_merged_seurat))
)

# Save to a gzipped TSV file
write.table(metadata_df,
            gzfile("../Data/Final_CRISPRi_Inputs/metadata.tsv.gz"),
            sep = '\t',
            row.names = FALSE,
            col.names = TRUE,
            quote = FALSE)
