...
def _filter_umi (df):
	...
	sgRNA_cutoff = [[_turn_point(i, df)] for i in list(df.index)]
	for i in range(0, len(sgRNA_cutoff)):
		df.iloc[i, :] = df.iloc[i, :].where(df.iloc[i, :] > sgRNA_cutoff[i][0], 0)
	return df
...
def main():
	file_path = "/oak/stanford/groups/larsms/Users/jgalante/External_Encode_E2G_Val/Xie/GSE129826/GSM3722739_sgRNA-enrichment_5K-sgRNAs_Batch_1_1_sgRNA_UMI.txt"
	...
	# Save to CSV
	df.to_csv("./batch_1_1_first.csv", index=False)
	print("DataFrame saved to", "batch_1_1_first.csv")
